...
class MyApp(object):
    ...
    def draw(self):
        self.draw_lines(self.width/2, self.height/2, \
                        min(self.width, self.height)/4,self.maxlength )
        self.maxlength=self.maxlength/2
    ...
    ## recursion ends!
    def draw_lines(self, centerx, centery, length, maxlength):
        ...
        if length>maxlength:
            self.draw_lines(centerx-length,centery,length/2,maxlength)
            self.draw_lines(centerx+length,centery,length/2,maxlength)
            self.draw_lines(centerx,centery-length,length/2,maxlength)
            self.draw_lines(centerx,centery+length,length/2,maxlength)
