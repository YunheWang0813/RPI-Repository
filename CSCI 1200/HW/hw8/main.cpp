...
void simple_test() {
  ...
  for (unsigned int i = 0; i < simple_points.size(); i++) {
    ...
    if (i == 0) { 
      ...
      std::cout << "\nafter inserting " << i+1 << " data points:" << std::endl;
      simple.plot(40,20);
//	  std::cout<<simple.height()<<std::endl;
...
void student_tests() {
  ...
  // --------------------------------------------------------
  // check for type<double, int> instead of <int, char>
  std::vector< std::pair<Point<double>,int> > simple_points;
  simple_points.push_back(std::make_pair(Point<double>(20,10), 1));
  simple_points.push_back(std::make_pair(Point<double>(10,5), 2));
  simple_points.push_back(std::make_pair(Point<double>(30,4), 3));
  simple_points.push_back(std::make_pair(Point<double>(11,15), 4));
  simple_points.push_back(std::make_pair(Point<double>(31,16), 5));
  simple_points.push_back(std::make_pair(Point<double>(5,3), 6));
  simple_points.push_back(std::make_pair(Point<double>(15,2), 7));
  simple_points.push_back(std::make_pair(Point<double>(4,7), 8));
  simple_points.push_back(std::make_pair(Point<double>(14,8), 9));
  simple_points.push_back(std::make_pair(Point<double>(25,1), 10));
  simple_points.push_back(std::make_pair(Point<double>(35,2), 11));
  simple_points.push_back(std::make_pair(Point<double>(26,7), 12));
  simple_points.push_back(std::make_pair(Point<double>(36,6), 13));
  simple_points.push_back(std::make_pair(Point<double>(3,13), 14));
  simple_points.push_back(std::make_pair(Point<double>(16,12), 15));
  simple_points.push_back(std::make_pair(Point<double>(4,17), 16));
  simple_points.push_back(std::make_pair(Point<double>(15,18), 17));
  simple_points.push_back(std::make_pair(Point<double>(25,13), 18));
  simple_points.push_back(std::make_pair(Point<double>(37,14), 19));
  simple_points.push_back(std::make_pair(Point<double>(24,19), 20));
  simple_points.push_back(std::make_pair(Point<double>(36,18), 21));


  // --------------------------------------------------------
  // the quad tree data structure starts out empty
  QuadTree<double,int> simple;
  assert (simple.size() == 0);
  // an empty tree has height == -1
  assert (simple.height() == -1); 
  ...
  for (unsigned int i = 0; i < simple_points.size(); i++) {
    ...
    if (i == 0) { 
      ...
      assert (simple.height() == 0); 
      // check that the newly inserted element can be found
      QuadTree<double,int>::iterator itr = simple.find(20,10);
      assert (itr != simple.end());
      // read the label & coordinates from the iterator
      assert (itr.getLabel() == 1);
      // dereference the iterator to get the point
      const Point<double> &pt = *itr;
      assert (pt.x == 20);
      assert (pt.y == 10);
    } else if (i <= 4) {
      // the next 4 additions for this simple all happen at the
...
//	  std::cout<<simple.height()<<std::endl;
      assert (simple.height() == 2);
      // check for an element that exists
      QuadTree<double,int>::iterator itr = simple.find(4,7);
      assert (itr != simple.end());
      assert (itr.getLabel() == 8);
      assert ((*itr).x == 4);
      assert ((*itr).y == 7);
      // check for a couple elements that aren't in the tree
      itr = simple.find(14,14);
  ...
  assert (simple.size() == 21);
  assert (simple.height() == 2);
  QuadTree<double,int>::iterator itr = simple.find(15,18);
  assert (itr != simple.end());
  assert (itr.getLabel() == 17);
  assert ((*itr).x == 15);
  assert ((*itr).y == 18);
  ...
  // note: this is a pre-order traversal, the same order as the 'sideways' tree above!!
  std::cout << "\nA depth-first traversal of the simple tree (should match sideways output!):" << std::endl;
  QuadTree<double,int>::iterator df_itr = simple.begin();
  int expected_depth_first_order[21] = 
    { 1,2,6,7,8,9,3,10,11,12,13,4,14,15,16,17,5,18,19,20,21 };
  for (int i = 0; i < 21; i++) {
    assert (df_itr != simple.end());
    // get the depth/level of this element in the tree (distance from root node!)
    int depth = df_itr.getDepth();
  ...
  // using the breadth-first iterator to traverse the data by level
  std::cout << "\nA breadth first traversal of the simple tree:";
  QuadTree<double,int>::bf_iterator bf_itr = simple.bf_begin();
  int expected_breadth_first_order[21] = 
...
// ==============================================================
// ==============================================================
//corner case: if there is a Point that is on the boundry, will it output correctly
void student_tests2() {
  std::cout << "Beginning student_tests()..." << std::endl;
  ...
  simple_points.push_back(std::make_pair(Point<int>(11,15), 'D'));
  simple_points.push_back(std::make_pair(Point<int>(31,16), 'E'));
  simple_points.push_back(std::make_pair(Point<int>(0,3), 'F'));
  simple_points.push_back(std::make_pair(Point<int>(15,0), 'G'));
  simple_points.push_back(std::make_pair(Point<int>(4,7), 'H'));
  simple_points.push_back(std::make_pair(Point<int>(14,8), 'I'));
  simple_points.push_back(std::make_pair(Point<int>(25,0), 'J'));
  simple_points.push_back(std::make_pair(Point<int>(40,2), 'K'));
  simple_points.push_back(std::make_pair(Point<int>(26,7), 'L'));
  simple_points.push_back(std::make_pair(Point<int>(36,6), 'M'));
  simple_points.push_back(std::make_pair(Point<int>(3,13), 'N'));
  simple_points.push_back(std::make_pair(Point<int>(16,12), 'O'));
