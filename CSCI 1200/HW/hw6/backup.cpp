	...
	Ship temp=ships[location.size()];
		...
		for(int i=0;i<r;i++){
			for(int j=0;j<c;j++){
				...
				for(int w=0;w<int(temp.getsize());w++){
					loc temploc2(i,j+w,1);
					bool test=false;
					for(int x=0;x<int(occupied.size());x++){
						if(temploc2==occupied[x]){
							judge0=false;
							test=true;
							break;
						}
				...
				if(judge0&&judge1){
					...
					for(int x=0;x<temp.getsize();x++){
						if(currentcol[j+x]+1>cols[j+x]&&cols[j+x]!=-1){
							judge2=false;
							break;
						}
					}
					...
					if(judge2){
						if(temp.getsize()+currentrow[i]<=rows[i]||rows[i]==-1){  //check if rows meet the requirement
		...
		for(int i=0;i<r;i++){
			for(int j=0;j<c;j++){
				...
				if(judge0&&judge1){
					...
					for(int x=0;x<temp.getsize();x++){
						if(currentrow[i+x]+1>rows[i+x]&&rows[i+x]!=-1){
							judge2=false;
							break;
						}
					}
					...
					if(judge2){
						if(temp.getsize()+currentcol[j]<=cols[j]||cols[j]==-1){  //check if rows meet the requirement
		...
		for(int i=0;i<r;i++){
			...
			for(int j=0;j<c;j++){
				...
				if(judge1){
					...
					if(judge2){
						if(temp.getsize()+currentcol[j]<=cols[j]||cols[j]==-1){  //check if cols meet the requirement
							currentcol[j]+=temp.getsize();        //increment currentcol
							for(int x=0;x<temp.getsize();x++){
								currentrow[i+x]+=1;     //increment currentrow
							}
