...
void PriorityQueue<T>::percolate_up(int i) {
	T element=m_heap[i];
  while (i >= 0 && element < m_heap[(i-1)/2]) {
	int temp=locations[m_heap[i]];
	locations[m_heap[i]]=locations[m_heap[(i-1)/2]];
	locations[m_heap[(i-1)/2]]=temp;
    std::swap(m_heap[i], m_heap[(i-1)/2]); // int(i -1)/2 is parent location
	i = (i-1)/2;
  }
}
...
void PriorityQueue<T>::percolate_down(int i) {
  while (i * 2 + 1 < int(m_heap.size())) {
    int child = i * 2 + 1;   // location of a parent's left child

    // find a parent's smaller child
    int swp = -1;
    if (child + 1 < int(m_heap.size()) &&  // check right child
	m_heap[child+1] < m_heap[child])
      swp = child + 1;
    else 
      swp = child;

    if (m_heap[swp] < m_heap[i]) {  // is child less than parent
      std::swap(m_heap[swp], m_heap[i]);
	  locations[m_heap[i]]=i;
	  locations[m_heap[swp]]=swp;
      i = swp;  // next time swap from here
    }
    else
      break;
    }
