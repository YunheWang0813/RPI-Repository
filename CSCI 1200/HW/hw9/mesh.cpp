...
bool Mesh::Collapse() {
  ...
  a->Average(b);		//now a becomes the middle point of edge ab
  
  std::vector<std::vector<Vertex*> > all_vertices;		//create the vector of vertices, for reconstruct triangles easily
  ...
  for (triangles_set::iterator itr = copy_tri.begin(); itr != copy_tri.end(); itr++) {
    ...
    if (t->HasVertex(a) && t->HasVertex(b)) {			//if the triangle has both a and b
	  for(int i=0;i<3;i++){
		  if(t->getVertex(i)!=a&&t->getVertex(i)!=b){
			  RemoveEdge(b,t->getVertex(i));		//remove the edge which connected with b
			  RemoveEdge(a,t->getVertex(i));
			  break;
		  }
	  }
	  RemoveEdge(a,b);
	...
	else if (t->HasVertex(a)){			//if the triangle only has a
		std::vector<Vertex*> vertices;
		for(int i=0;i<3;i++){
			if(t->getVertex(i)!=a){
				vertices.push_back(t->getVertex(i));		//store the required vertex
				RemoveEdge(a,t->getVertex(i));			//remove the specific edges at the same time
			}
		}
	...
	else if (t->HasVertex(b)){			//if the triangle only has b
		std::vector<Vertex*> vertices;
		for(int i=0;i<3;i++){
			if(t->getVertex(i)!=b){
				vertices.push_back(t->getVertex(i));		//store the required vertex
				RemoveEdge(b,t->getVertex(i));			//remove the specific edges at the same time
			}
		}
		RemoveEdge(vertices[0],vertices[1]);
		vertices.push_back(a);
  ...
  for(int i=0;i<int(all_vertices.size());i++){
	  if(right-side-up(all_vertices[i][0],all_vertices[i][1],all_vertices[i][2]){
	  AddTriangle(all_vertices[i][0],all_vertices[i][1],all_vertices[i][2]);
	  }
  }
    
  std::set<Edge*> tempedges;
  std::set<Edge*> newedges;
  CollectEdgesWithVertex(a,newedges);
  ReCalculateEdges(newedges);
  
  for(std::set<Edge*>::iterator itr=newedges.begin();itr!=newedges.end();itr++){
	  if((*itr)->getV1()==a){
		  CollectEdgesWithVertex((*itr)->getV2(),tempedges);
	  }
	  else if((*itr)->getV2()==a){
		  CollectEdgesWithVertex((*itr)->getV1(),tempedges);
	  }
  }
  ReCalculateEdges(tempedges);
...
//  std::cout<<a->getEdges().size()<<std::endl;
