...
*/
  ...
  */
  ...
  std::ifstream ifs("hansel_and_gretel.txt");
  ...
  insert_result = set1.insert( to_insert );
  std::unordered_set<std::string> set1ex;

  while(ifs>>to_insert){
	  set1ex.insert(to_insert);
  }

  for(std::unordered_set<std::string>::iterator it=set1ex.begin();it!=set1ex.end();it++){
	  set1.insert(*it);
  }

  std::cout << "The unique words number is " << set1.size() << '\n'
	    << "Here is the table: \n";
  set1.print( std::cout );
