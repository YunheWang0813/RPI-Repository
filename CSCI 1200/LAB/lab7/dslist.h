...
public:
  ...
  list_iterator() : ptr_(NULL), tailex(NULL) {}
  list_iterator(Node<T>* p, Node<T>* t) : ptr_(p) ,tailex(t){}
...
private:
  ...
  Node<T>* tailex;   //determine if ptr is nullptr
};
...
public:
  ...
  iterator end(){return iterator(NULL,tail_);}
...
/*template <class T>
typename dslist<T>::iterator dslist<T>::end() {
	Node<T>* temp;
	temp=tail_->next_;
	return temp;
}*/
