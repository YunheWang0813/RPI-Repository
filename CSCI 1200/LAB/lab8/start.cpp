...
void printpaths(Point p, vector<vector<bool> > const& blocked_grid,vector<Point> store){
	if(blocked_grid[p.x][p.y]){
		return;
	}
	
	store.push_back(Point(p.x,p.y));
	...
	printpaths(Point(p.x-1,p.y),blocked_grid,store);
	cout<<endl;
	printpaths(Point(p.x,p.y-1),blocked_grid,store);
}
...
int main(int argc, char* argv[]) {
  ...
  vector<Point> store;
  printpaths(Point(start_x,start_y),blocked_grid,store);
