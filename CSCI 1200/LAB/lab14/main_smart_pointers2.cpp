...
*/
  ...
  std::vector<Balloon*> balloonini;
  balloonini.push_back(jacob);
  balloonini.push_back(katherine);
  balloonini.push_back(larry);
  balloonini.push_back(miranda);
  balloonini.push_back(nicole);
  
  std::set<Balloon*> balloons;
  for(int n=0;n<balloonini.size();n++){
  for(int i=0;i<balloonini[n]->numRopes();i++){
	  Balloon* temp=jacob->getRope(i);
	  if(balloons.find(temp)==balloons.end()){
		  balloons.insert(temp);
	  }
  }
  }
...
//  deleteAll(jacob);
  ...
  for(std::set<Balloon*>::iterator i=balloons.begin();i!=balloons.end();i++){
	  delete (*i);
  }
